...
| `ireps_values`, `ireps_symbols` | irrep dependent | Quantum numbers and symbols identifying the irrep rows. The legacy spelling `ireps` is retained for compatibility. |
| `qpoint_z` | scalar | Reduced axial q coordinate supplied with `--qpoint_z`. |
| `operation_labels` | `(N_op,)` | Operation corresponding to each `characters` column, including fractional translations. |
| `operation_class_labels` | `(N_op,)` | Class label repeated in character-table column order for convenient reporting. |
| `conjugacy_class_labels` | `(N_class,)` | One label per conjugacy class, for example `2 S4(+90deg,axis=z)`. |
| `conjugacy_class_sizes` | `(N_class,)` | Number of operations in each conjugacy class. |
| `class_characters` | `(N_irrep, N_class)` | Character table compressed to one column per class at Gamma. |

At Gamma, `class_characters` is generated only after verifying that every character is constant within its conjugacy class. Away from Gamma it is empty because nonsymmorphic Bloch phases may give projective representations; `q_preserving_mask` and the `little_group_*` fields provide the corresponding little-group information instead.

Additional metadata such as class IDs, representatives, members, and generator words is also included for programmatic verification. With `--enable_rep_matrix`, `D_irrep_0`, `D_irrep_1`, ... contain the representation matrices.
